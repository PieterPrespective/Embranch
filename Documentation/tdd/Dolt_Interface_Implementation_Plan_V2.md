...
public class DoltTools
{
    ...
    [McpTool("dolt_status", "Get repository status including uncommitted local changes")]
    public async Task<ToolResult> StatusAsync()
    {
        ...
        return new ToolResult
        {
            Content = new
            {
                ...
                localChanges = new
                {
                    hasChanges = status.LocalChanges.HasChanges,
                    newDocuments = status.LocalChanges.NewDocuments.Count,
                    modifiedDocuments = status.LocalChanges.ModifiedDocuments.Count,
                    deletedDocuments = status.LocalChanges.DeletedDocuments.Count,
                    totalChanges = status.LocalChanges.TotalChanges
                },
                doltChanges = status.HasUncommittedDoltChanges
            }
        };
    }
    ...
    public async Task<ToolResult> InitAsync(
        [McpParameter("collection", "ChromaDB collection to enable version control for", required: true)] string collection,
        [McpParameter("message", "Initial commit message", required: false)] string message = "Initial import from ChromaDB")
    {
        var result = await _syncManager.InitializeVersionControlAsync(collection, message);

        return new ToolResult
        {
            Content = new
            {
                success = result.Status == InitStatus.Completed,
                documentsImported = result.DocumentsImported,
                commitHash = result.CommitHash,
                message = result.Status == InitStatus.Completed 
                    ? $"Version control initialized with {result.DocumentsImported} documents"
                    : result.ErrorMessage
            }
        };
    }

    // ==================== Commit (with Chroma → Dolt staging) ====================

    [McpTool("dolt_commit", "Stage local ChromaDB changes and commit to version control")]
    public async Task<ToolResult> CommitAsync(
        [McpParameter("message", "Commit message", required: true)] string message,
        [McpParameter("auto_stage", "Automatically stage local ChromaDB changes", required: false)] bool autoStage = true)
    {
        var result = await _syncManager.ProcessCommitAsync(message, autoStageFromChroma: autoStage);
        ...
        return new ToolResult
        {
            Content = new
            {
                ...
                commitHash = result.CommitHash,
                stagedFromChroma = result.StagedFromChroma,
                synced = new
                {
                    added = result.Added,
        ...
        [McpParameter("remote", "Remote name", required: false)] string remote = "origin")
    {
        ...
        var status = await _syncManager.GetStatusAsync();
        if (status.HasUncommittedChromaChanges)
        {
            return new ToolResult
            {
                Content = new
                {
                    ...
                    message = $"You have {status.LocalChanges.TotalChanges} uncommitted local changes. Commit them first with dolt_commit.",
                    localChanges = new
                    {
                        newDocuments = status.LocalChanges.NewDocuments.Count,
                        modifiedDocuments = status.LocalChanges.ModifiedDocuments.Count,
    ...
    // ==================== Pull (with local change warning) ====================

    [McpTool("dolt_pull", "Pull from DoltHub and sync to ChromaDB")]
    public async Task<ToolResult> PullAsync(
        [McpParameter("remote", "Remote name", required: false)] string remote = "origin",
        [McpParameter("force", "Force pull, discarding local changes", required: false)] bool force = false)
    {
        var result = await _syncManager.ProcessPullAsync(remote, force);

        if (result.Status == SyncStatus.LocalChangesExist)
        {
            return new ToolResult
            {
                Content = new
                {
                    ...
                    localChanges = new
                    {
                        newDocuments = result.LocalChanges.NewDocuments.Count,
                        modifiedDocuments = result.LocalChanges.ModifiedDocuments.Count,
                        deletedDocuments = result.LocalChanges.DeletedDocuments.Count
                    },
                    hint = "Commit your changes first with dolt_commit, or use force=true to discard them"
                }
            };
        }
    ...
    // ==================== Checkout (with local change warning) ====================
    ...
    public async Task<ToolResult> CheckoutAsync(
        ...
        [McpParameter("force", "Force checkout, discarding local changes", required: false)] bool force = false)
    {
        var result = await _syncManager.ProcessCheckoutAsync(branch, create, force);
        ...
        if (result.Status == SyncStatus.LocalChangesExist)
        {
            return new ToolResult
            {
                Content = new
                {
                    ...
                    hint = "Commit your changes first, or use force=true to discard them"
                }
            };
        }
